...
latencies are simulated with the fixed, documented values below and are
**consumed through the injectable clock** (``Clock.sleep``): they occupy task
duration, timeout budget, request pacing, and cell wall time exactly like
real delays (decision D-0010), while remaining separable from model-serving
time because every invocation's latency is also recorded in the tool trace.
Tests inject a virtual clock so the suite stays fast and deterministic.

The reference evidence path (five evidence tools plus the terminal tool)
consumes a fixed total of 85 ms: 5 + 20 + 30 + 15 + 10 + 5.
"""
...
#: methodology/workload-definition.md). Every invocation of a tool consumes
#: exactly this latency via the injected clock and records it in the trace.
...
TOOL_SPECS: dict[str, dict[str, dict[str, type | tuple[type, ...]]]] = {
    ...
    "recommend_remediation": {
        "required": {"diagnosis_id": str, "rationale": str, "remediation_id": str},
        "optional": {},
    },
}

#: The terminal tool: calling it ends the task with the agent's structured
...
def validate_tool_call(name: str, arguments: dict[str, Any]) -> None:
    ...
    for arg, value in arguments.items():
        ...
        # bool is a subclass of int: reject booleans where integers are required.
        if expected is int and isinstance(value, bool):
            raise InvalidToolArgumentsError(
                f"{name}: argument {arg!r} must be an integer, not a boolean"
            )
...
class SimulatedToolbox:
    ...
    ``clock`` consumes each tool's fixed simulated latency.
    """
    ...
    def _tool_retrieve_runbook(self, key: str) -> dict[str, Any]:
        ...
        return {
            ...
            "found": True,
            "runbook": runbook,
            "diagnosis_candidates": list(self._scenario.candidate_diagnoses),
        }
